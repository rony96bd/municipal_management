    ...
    @forelse ($sidebars as $sidebar)
        @if ($sidebar && $sidebar->forigen)
            @if ($sidebar->forigen_type === App\Models\officials\officials::class)
                <div class="page-repeater bradius-6px cursor-drag position-relative" style="border:none"
                    data-id="{{ $sidebar->id }}">
                    ...
                    <form action="{{ route('delete-sidebar', $sidebar->id) }}" method="POST"
                        onsubmit="return confirm('আপনি কি নিশ্চিতভাবে মুছে ফেলতে চান?');" style="display:inline;">
                        @csrf
                        @method('DELETE')
                        <button type="submit" class="delete-sidebar">
                            ☓
                        </button>
                    </form>
                </div>
        ...
        @endif
        @if ($sidebar->gap)
            <div class="page-repeater bradius-6px cursor-drag sidebar-gap padar-20 flex row center position-relative"
                data-id="{{ $sidebar->id }}">
                <p>গ্যাপ</p>
                <form action="{{ route('delete-sidebar', $sidebar->id) }}" method="POST"
                    onsubmit="return confirm('আপনি কি নিশ্চিতভাবে মুছে ফেলতে চান?');" style="display:inline;">
        ...
        @if ($sidebar->sidebar_title)
            <div class="page-repeater sidebar-title-area flex row gap-10 jst-ace full-width padar-10 position-relative marb-10 cursor-drag position-relative"
                data-id="{{ $sidebar->id }}">
